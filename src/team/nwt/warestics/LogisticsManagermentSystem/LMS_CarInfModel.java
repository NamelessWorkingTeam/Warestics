...
public class LMS_CarInfModel extends AbstractTableModel {
    ...
    //通过传递的sql语句来获得数据模型
    public LMS_CarInfModel(String sql)
    {
    	//建立表头  
    ...
    public LMS_CarInfModel()
    {
		...
		RowData=new Vector(); 
		sql = "SELECT * FROM tb_car";						// 此处填写要执行的语句
	    db = new MySQLConnect(sql);							// 新建一个数据库连接
	    try {
			ret = db.pst.executeQuery();					// 执行sql语句，得到结果集
			while (ret.next()) {
